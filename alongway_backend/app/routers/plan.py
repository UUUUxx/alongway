...
async def plan_endpoint(request: PlanRequest) -> PlanResponse:
    ...
    if settings.debug:
        logger.info(
            ...
            "budget=%s start=%s end=%s current=%s "
            ...
            request.travel_mode,
            request.budget,
            request.start_location.model_dump_json() if request.start_location else None,
            request.end_location.model_dump_json() if request.end_location else None,
            request.current_location.model_dump_json() if request.current_location else None,
