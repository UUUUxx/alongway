...
| **POI 搜索**      | 本地 DB 优先 → 结果不足时调用高德周边搜索并入库；高德也无结果时按搜索中心生成临时 Mock POI |
| **Deal 搜索**     | 按类别/预算/销量排序；缺少团购时按 POI 类型生成确定性 Mock 团购并入库                       |
| **路线计算**      | 高德真路网 API（步行/骑行/驾车）→ Haversine 兜底                                           |
| **地理编码**      | 地址→坐标代理，保护 AMAP_KEY 不暴露前端                                                    |
| **高德 POI 清洗** | 保留餐饮/娱乐/生活服务；过滤停车场/公司/住宅等                                              |
| **Mock 团购**     | 确定性模板生成（非随机），同一 POI 每次结果一致；覆盖内置、高德补拉和临时 Mock POI          |
| **当前适配城市**  | 目前只适配武汉；前端默认 city=武汉，地理编码、POI 搜索和推荐示例均按武汉场景调试            |
...
```text
从华中科技大学，去世界城广场，我想喝茶百道，吃寿司，再吃一块蛋糕
```
...
```text
我想从华中师范大学，去武汉大学，我想顺路吃早餐再买杯咖啡，吃点小吃
```
...
| 维度                | 数值                                                                           |
| ------------------- | ------------------------------------------------------------------------------ |
| 内置 Seed POI 总数  | 18 (武汉 12 + 上海 6)，启动时由 `app.seed` 写入数据库                        |
| 内置 Seed Deal 总数 | 26 (武汉 18 + 上海 8)，启动时由 `app.seed` 写入数据库                        |
| 运行时 POI 补充     | 本地结果不足时调用高德 POI；高德结果会清洗后以 `amap_` 前缀入库              |
| 运行时 Mock POI     | 本地和高德都无结果且存在搜索中心坐标时，会生成 `mock_` 前缀的临时 POI 并入库 |
| 运行时 Mock Deal    | 高德补拉 POI、临时 Mock POI 或缺少团购的 POI，会按类型生成确定性 Mock 团购     |
